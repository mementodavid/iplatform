<?php
	...
	if( isset( $_POST[ "topic_one" ] ) && $_POST[ "topic_one" ] == TRUE) {
		$topic_one = $_POST[ "topic_one" ];
	} else {
		echo 'not set';
	}

	if( isset( $_POST[ "topic_two" ] ) && $_POST[ "topic_two" ]  == TRUE ) {
		$topic_two = $_POST[ "topic_two" ];
	} else {
		echo 'not set';
	}
	...
	if( isset( $topic_one ) && isset( $topic_two ) ) {
		$topics = array( $topic_one, $topic_two );
	} elseif( isset( $topic_one ) && !isset( $topic_two ) ) {
		$topics = $topic_one;
	} else {
		$topcs = $topic_two;
	}
...
<?php
		...
		if( $category ) {
			$query .= "
				INNER JOIN insights_categories
				ON insights.id = insights_categories.insight_id";
		}

		if( $topics ) {
			$query .= "
				INNER JOIN insights_topics
				ON insights.id = insights_topics.insight_id";
		}
		...
		// WHERE
		if( $category) {
			$query .= " WHERE insights_categories.cat_id = $category";
		}
		...
		if( $topics ) {
			...
			$query .="insights_topics.topic_id IN ( $topics )";
		}
...
/*
		...
		<div>
			<table>
				...
				<tr>
					<td>
						<a href="/iplatform">Recalibrate your briefing</a>
					</td>
				</tr>
			</table>
